...
T&&(I+=ca,h*=1+sin(I)*T);h|=0;8>h&&(h=8);E||(m+=N,0>m?m=0:0.5<m&&(m=0.5));if(++v>F)switch(v=0,++U){case 1:F=f;break;case 2:F=d}switch(U){case 0:w=v*da;break;case 1:w=1+2*(1-v*ea)*ba;break;case 2:w=1-v*fa;break;case 3:w=0,G=!0}R&&(D+=aa,s=D|0,0>s?s=-s:1023<s&&(s=1023));P&&Q&&(r*=Q,1E-5>r?r=1E-5:0.1<r&&(r=0.1));q=0;for(var ga=8;ga--;){p++;if(p>=h&&(p%=h,3==E))for(x=y.length;x--;)y[x]=2*Math.random()-1;switch(E){case 0:b=p/h<m?0.5:-0.5;break;case 1:b=1-2*(p/h);break;case 2:b=p/h;b=0.5<b?6.28318531*
(b-1):6.28318531*b;b=0>b?1.27323954*b+0.405284735*b*b:1.27323954*b-0.405284735*b*b;b=0>b?0.225*(b*-b-b)+b:0.225*(b*b-b)+b;break;case 3:b=y[abs(32*p/h|0)]}P&&(x=u,n*=X,0>n?n=0:0.1<n&&(n=0.1),Y?(t+=(b-u)*n,t*=a):(u=b,t=0),u+=t,B+=u-x,b=B*=1-r);R&&(C[H%1024]=b,b+=C[(H-s+1024)%1024],H++);q+=b}q=0.125*q*w*Z;c[k]=1<=q?32767:-1>=q?-32768:32767*q|0}return O}};
...
// All sounds should be in the format used by jsfxr. This sound manage supports multiple channels,
// so you can set the 'channels' variable when adding the sound to play that sound multiple times
// concurrently
var Sound = function() {
  var _ = this;
  // Sounds object
  ...
  // Add sound
  // i: sound key/id
  // x: sound settings array
  // c: channels
  _.a = function(i, x, c) {
    c = c || 1;
    _.s[i] = new MCAudio(x, c);
  };
...
// Multi-channel audio
var MCAudio = function(x, c) {
  var _ = this;
  _.c = [];
  for (var i=c; i--; ) {
    _.c.push(new SAudio(x));
  }

  _.p = function() {
    for (var i=_.c.length; i--; ) {
      if (!_.c[i].b) {
        _.c[i].p();
        break;
      }
...
var SAudio = function(x) {
  var _ = this;
  _.b = 0; // Busy?
  _.a = new Audio();
  _.a.src = jsfxr(x);
  _.a.addEventListener('playing', function() { _.b = 1; });
  _.a.addEventListener('ended', function() { _.b = 0; });
